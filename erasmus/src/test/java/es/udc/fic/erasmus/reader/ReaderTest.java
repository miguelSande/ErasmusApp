...
public class ReaderTest extends WebAppConfigurationAware {
	...
	public void testReaderUni() throws IOException {
		List<University> expected = readerU.readUniExcel(this.getClass().getResource("/destinos.XLS").getFile());
	...
	public void testReaderStudent() throws IOException {
		List<Request> expected = readerS.readRequestExcel(this.getClass().getResource("/pedidos.xlsx").getFile());
		assertEquals(37, expected.size());
		List<Student> expec = readerS.readStudentExcel(this.getClass().getResource("/pedidos.xlsx").getFile());
		assertEquals(15, expec.size());
	}
	
	@Test  //problemas con el excel universidad inexistente
	public void testProcess() throws IOException {
		List<University> universities = readerU.readUniExcel(this.getClass().getResource("/destinos.XLS").getFile());
		for (University u: universities) {
			uniRepo.save(u);
		}
		University une = uniRepo.findByName("Kookmin University");
		assertEquals("Kookmin University", une.getName());
		List<Student> students = readerS.readStudentExcel(this.getClass().getResource("/pedidos.xlsx").getFile());
		for (Student s: students) {
			stRepo.save(s);
		}
		Student sample = stRepo.findByName("Gallardo Pedrosa, María");
		assertEquals("7.0783", sample.getNote().toString());
		List<Request> requests = readerS.readRequestExcel(this.getClass().getResource("/pedidos.xlsx").getFile());
